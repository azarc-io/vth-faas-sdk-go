	...
	tests := []struct {
		...
		},
		{
			...
			chainFn: func() (*spark.Chain, *stageBehaviour) {
				sb := NewStageBehaviour(t, "stage1", "stage2").
					Change("stage1", nil,
						sdk_errors.NewStageError(errors.New("err-stage1"), sdk_errors.WithErrorType(v12.ErrorType_ERROR_TYPE_SKIP)))
				chain, err := spark.NewChain(
					spark.NewNode().
						Stage("stage1", stageFn("stage1", sb)).
						Stage("stage2", stageFn("stage2", sb), spark.WithStageStatus("stage1", v12.StageStatus_STAGE_STATUS_COMPLETED)).
						Build()).
					Build()
				assert.Nil(t, err, "error creating spark node chain: %v", err)
				return chain, sb
		...
		},
		{
			...
			chainFn: func() (*spark.Chain, *stageBehaviour) {
				...
				chain, err := spark.NewChain(
					spark.NewNode().
						Stage("stage1", stageFn("stage1", sb)).
						Stage("stage2", stageFn("stage2", sb), spark.WithStageStatus("stage1", v12.StageStatus_STAGE_STATUS_SKIPPED)).
						Build()).
					Build()
				assert.Nil(t, err, "error creating spark node chain: %v", err)
				return chain, sb
		...
		},
		{
			...
			chainFn: func() (*spark.Chain, *stageBehaviour) {
				sb := NewStageBehaviour(t, "stage1").
					Change("stage1", nil,
						sdk_errors.NewStageError(errors.New("err-stage1"), sdk_errors.WithErrorType(v12.ErrorType_ERROR_TYPE_FAILED_UNSPECIFIED)))
				chain, err := spark.NewChain(
					spark.NewNode().
						Stage("stage1", stageFn("stage1", sb)).
						Build()).
		...
		},
		{
			...
			chainFn: func() (*spark.Chain, *stageBehaviour) {
				sb := NewStageBehaviour(t, "stage1").
					Change("stage1", nil,
						sdk_errors.NewStageError(errors.New("err-stage1"), sdk_errors.WithErrorType(v12.ErrorType_ERROR_TYPE_CANCELLED)))
				chain, err := spark.NewChain(
					spark.NewNode().
						Stage("stage1", stageFn("stage1", sb)).
						Build()).
			...
			chainFn: func() (*spark.Chain, *stageBehaviour) {
				sb := NewStageBehaviour(t, "stage1", "compensate").
					...
					Change("compensate", nil,
						sdk_errors.NewStageError(errors.New("err-compensate"), sdk_errors.WithErrorType(v12.ErrorType_ERROR_TYPE_CANCELLED)))
				chain, err := spark.NewChain(
					spark.NewNode().
						Stage("stage1", stageFn("stage1", sb)).
						Compensate(spark.NewNode().Stage("compensate", stageFn("compensate", sb)).Build()).
			...
			chainFn: func() (*spark.Chain, *stageBehaviour) {
				sb := NewStageBehaviour(t, "stage1", "cancel").
					Change("stage1", nil,
						sdk_errors.NewStageError(errors.New("err-stage1"), sdk_errors.WithErrorType(v12.ErrorType_ERROR_TYPE_CANCELLED))).
					Change("cancel", nil,
						sdk_errors.NewStageError(errors.New("err-cancel"), sdk_errors.WithErrorType(v12.ErrorType_ERROR_TYPE_RETRY)))
				chain, err := spark.NewChain(
					spark.NewNode().
						Stage("stage1", stageFn("stage1", sb)).
						Cancelled(spark.NewNode().Stage("cancel", stageFn("cancel", sb)).Build()).
		...
		},
		{
			...
			chainFn: func() (*spark.Chain, *stageBehaviour) {
				sb := NewStageBehaviour(t, "stage1").Change("stage1", nil, sdk_errors.NewStageError(errors.New("stage1"), sdk_errors.WithErrorType(v12.ErrorType(500))))
				chain, err := spark.NewChain(
					spark.NewNode().
						Stage("stage1", stageFn("stage1", sb)).
						Build()).
